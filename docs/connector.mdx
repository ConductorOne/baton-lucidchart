...
†Delete and the account actions require a Lucid Enterprise SCIM bearer token (`lucid-scim-token`). Without it, sync and account creation still work, but these capabilities are unavailable.

When a new account is created by C1, the account's password will be sent to a [vault](/product/admin/vaults). 

### Account actions

Three account actions run against the SCIM surface, so each one needs the `lucid-scim-token` described above:
