...
| Resource | Sync | Provision | Delete | Actions |
| :--- | :--- | :--- | :--- | :--- |
| Accounts | <Icon icon="square-check" iconType="solid"  color="#c937ae"/> | <Icon icon="square-check" iconType="solid"  color="#c937ae"/>* | <Icon icon="square-check" iconType="solid"  color="#c937ae"/>† | <Icon icon="square-check" iconType="solid"  color="#c937ae"/>† |
...
Three account actions run against the SCIM surface, so each one needs the `lucid-scim-token` described above (see [Create a SCIM token](#create-a-scim-token)):

| Action | What it does |
| :--- | :--- |
| `enable_user` | Reactivates an account (SCIM `active=true`). |
| `disable_user` | Deactivates an account (SCIM `active=false`). Soft and reversible. |
| `update_user` | Updates an account's `firstName`, `lastName`, `email`, `username` or `roles`. |
...
stringData:
  ...
  # Optional: required for account delete and the enable_user, disable_user and
  # update_user actions. Enterprise-only; everything else works without it.
  BATON_LUCID_SCIM_TOKEN: <Lucidchart SCIM token>
