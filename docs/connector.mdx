...
¹ Licenses is an opt-in resource type and is not synced by default. To enable it, select **Licenses** in the resource types to sync when configuring the connector. Syncing licenses requires the `licenses:admin.readonly` OAuth scope, which is only available on Lucidchart accounts with Enterprise licenses (see the OAuth scopes step below).
...
If you want to use the Lucidchart connector to provision accounts, give the token the `account.user` scope.
   ...
   If you want to sync license and subscription data (the opt-in **Licenses** resource type), add the `licenses:admin.readonly` scope. This scope requires a Lucidchart account with an Enterprise-tier license; Lucid's authorize endpoint returns `invalid_scope` for non-Enterprise tenants.
</Step>
<Step>
